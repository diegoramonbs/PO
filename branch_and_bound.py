...
class BranchAndBoundTest(unittest.TestCase):
	...
	def test_model_1(self):
		# optimal 68, variables [4, 0]
		model = Model()
		model.A = np.array([[10,7],[1,1]])
		model.b = np.array([40,5])
		model.c = np.array([17,12])
		model.constraints = np.array(["<=", "<="])
		model = branch_and_bound(model, verbose=True)
		self.assertEqual(model.primal_objective, 68)

	def test_model_2(self):
		# Optimal 27,  variables [3, 2]
		model = Model()
		model.A = np.array([[1,1],[4,7],[0,1]])
		model.b = np.array([5,28,2])
		model.c = np.array([5,6])
		model.constraints = np.array(["<=", "<="])
		model = branch_and_bound(model, verbose=False)
		self.assertEqual(model.primal_objective, 27)

	def test_model_3(self):
		# Optimal 8, variables []
		model = Model()
		model.A = np.array([[2, 6],[2, -2]])  
		model.b = np.array([15, 3])
		model.c = np.array([2, 3])
		model.constraints = np.array(["<=", "<="])
		model = branch_and_bound(model, verbose=False)
		self.assertEqual(model.primal_objective, 8)


	def test_model_4(self):
		# Optimal 6, variables []
		model = Model()
		model.A = np.array([[1,1],[2,0]])
		model.b = np.array([4,5])
		model.c = np.array([2,1])
		model.constraints = np.array(["<=", "<="])
		model = branch_and_bound(model, verbose=False)
		self.assertEqual(model.primal_objective, 6)
